...
function AppointmentPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    setLoading(true)
    const [doctorsResult, appointmentsResult, profilesResult] = await Promise.all([
      supabase.from('profiles').select('user_id, full_name, specialization, experience_years, availability, license_number').eq('role', 'doctor').order('full_name'),
      supabase.from('appointments').select('id, patient_id, doctor_id, appointment_date, appointment_time, reason, notes, status, created_at, updated_at').order('appointment_date').order('appointment_time'),
      supabase.from('profiles').select('user_id, full_name, email, role').in('role', role === 'patient' ? ['doctor'] : ['patient', 'doctor']),
    ])
    const firstError = doctorsResult.error || appointmentsResult.error || profilesResult.error
  ...
  async function bookAppointment(event) {
    ...
    setStatus(null)
    const { error } = await supabase.from('appointments').insert({ patient_id: user.id, doctor_id: selectedDoctor.user_id, appointment_date: selectedDate, appointment_time: selectedTime, reason: reason.trim() || null, status: 'Pending' })
    if (error) {
  ...
  async function updateAppointment(id, nextStatus) {
    if (!['Pending', 'Confirmed', 'Completed', 'Cancelled'].includes(nextStatus)) return
